			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td width="93%" class="ct_ttl01">
							<%-- <%= (menu.equals("manage") ? "상품관리" : "상품목록조회") %> --%>
							<%-- 메뉴 뭘로 옴? ${param.menu}<br/><br/> --%>
							<%-- 지금 접속한사람??? ${user.role}<br/><br/> --%>
				...
				<c:choose>
					<c:when test="${product.proTranCode=='1  '}">
						...
						<c:if test="${param.menu =='manage' }">
							<a href="/updateTranCodeByProd.do?prodNo=${product.prodNo}&tranCode=2&page=${ !empty param.page ? search.page : '' }">배송하기</a>
				 		</c:if>
					</c:when>
